...
(function () {
  ...
  // 認證事件的序列化與去重：
  // Supabase 初次載入會連續送出多個事件（INITIAL_SESSION、可能還有 TOKEN_REFRESHED/SIGNED_IN），
  // 若每個事件都各自跑一次 onSession，會有「兩個 getUser() 併發刷新同一把 refresh token」的競態，
  // 導致 token 輪替被判定為重複使用 → 整串 session 被伺服器撤銷 →「Ctrl+F5 後再也登不進去、要清站台資料」。
  // 解法：① 用 authChain 把每次處理串成佇列，絕不併發；② 同一個使用者狀態沒變就不重跑。
  let authChain = Promise.resolve();   // 進行中的處理鏈（序列化用）
  let currentUid = undefined;          // 目前畫面已套用的使用者 id（null = 未登入）
  ...
  async function onSession(session) {
    ...
    if (uid === currentUid) return;   // 狀態未變（含 TOKEN_REFRESHED 等重複事件）→ 不重跑、不再呼叫 getUser
    if (uid) {
      ...
      if (ue || !u || !u.user) {
        currentUid = null;
        me = null;
        await sb.auth.signOut();   // 清掉本機殘留的失效登入
        // 直接切回登入畫面（後續 signOut 的 null 事件會因去重被略過，故這裡自己切）
        $("#app").classList.add("hidden");
        $("#login").classList.remove("hidden");
        $("#loginErr").textContent = "登入已逾期，請重新登入。";
        return;
      }
      currentUid = u.user.id;
      me = u.user;
  ...
  function renderList() {
    ...
    // 排序：依使用者選的方式。時間欄位缺值時退回 0，避免 NaN。
    const sortEl = $("#fSort");
    const sortBy = sortEl ? sortEl.value : "updated";
    const tval = (s) => (s ? new Date(s).getTime() : 0);
    const gRank = { A: 0, B: 1, C: 2 };
    const byUpdated = (a, b) => tval(b.updated_at) - tval(a.updated_at);
    rows.sort((a, b) => {
      if (sortBy === "created") return tval(b.created_at) - tval(a.created_at) || byUpdated(a, b);
      if (sortBy === "grade") {
        const d = (gRank[a.grade] ?? 9) - (gRank[b.grade] ?? 9);
        return d || byUpdated(a, b);                       // 同分級內以最近更新排序
      }
      if (sortBy === "mine" || sortBy === "other") {
        const am = a.owner_id === (me && me.id) ? 0 : 1, bm = b.owner_id === (me && me.id) ? 0 : 1;
        const d = sortBy === "mine" ? am - bm : bm - am;    // 我的優先 / 夥伴優先
        return d || byUpdated(a, b);
      }
      return byUpdated(a, b);                               // 預設：最近更新
    });
    if (!rows.length) {
  ...
  function cardHTML(c) {
    ...
    const gCls = c.grade ? "g-" + c.grade : "";   // 依分級給卡片底色（見 styles.css .ccard.g-A/B/C）
    return `<div class="ccard ${gCls}" data-id="${c.id}" style="border-left:5px solid ${oc}">
...
【重要・立場判別】對話中「我」是房仲，對方是客戶。只整理「客戶本人」的需求，
不要把「我(房仲)主動傳給客戶的物件、地址、社區、廣告或介紹」當成客戶的需求。
判斷原則：是客戶自己說出口想要的、主動詢問的、或明確表示有興趣／想看／看過的，才算需求；
單純是我貼給他、推銷給他、而客戶沒有正面回應的物件地址或社區，一律不要列入；
客戶已明確表示「不要／pass／不考慮」的社區或地點，也不要列入需求。

【區域 vs 門牌】「某一間待售物件的門牌地址(巷弄號樓)」不是客戶想要的區域。
areas 只放客戶自己講的「地點條件」——行政區、捷運站、商圈、學區、步行範圍這種範圍描述；
任何具體門牌(如「文化二路一段230巷」「民權路12巷」)即使客戶對那間有興趣，也不要放進 areas，
那種具體地址若需要保留，寫進 note 即可。

{
  ...
  "budget_min": 總價下限萬元整數,
  "budget_max": 總價上限萬元整數,
  "areas": ["客戶自己講的地點條件:行政區/捷運站/商圈/步行範圍(不含任何單一物件門牌)"],
  "communities": ["客戶主動表示有興趣或看過的社區(排除我推銷的、及客戶已說pass/不考慮的)"],
  "room_min": 房數下限,
  "room_max": 房數上限,
  "ping_min": 坪數下限,
  "ping_max": 坪數上限,
...
`;
  ...
  // 寬鬆解析「被截斷的 JSON」：AI 輸出常被截斷（少收尾的 } 、或 note 字串打到一半就斷），
  // 這裡逐字掃描，記錄字串狀態與 {/[ 堆疊，最後補上缺的引號與收尾括號再解析。
  function tryParseLooseJson(src) {
    try { return JSON.parse(src); } catch (_e) { /* 直接解析失敗 → 往下修補 */ }
    let inStr = false, esc = false, lastSig = -1;
    const stack = [];
    for (let i = 0; i < src.length; i++) {
      ...
      if (!/\s/.test(c)) lastSig = i;     // 記住最後一個非空白字元位置
    }
    let out = src.slice(0, lastSig + 1);                 // 去掉尾端空白/換行
    if (inStr) out += '"';                               // 字串被截斷 → 補回結尾引號
    out = out.replace(/,?\s*"[^"]*"\s*:\s*$/, "");       // 去掉尾端不完整的「鍵：」(冒號後沒值就斷了)
    out = out.replace(/,\s*$/, "");                      // 去掉尾端多餘逗號
    for (let i = stack.length - 1; i >= 0; i--) out += stack[i] === "{" ? "}" : "]";  // 補回對應收尾
  ...
  function parsePasted(text) {
    ...
    // 1a) 標準情形：第一個 { 到最後一個 }（可去掉 JSON 後面多餘的說明文字）
    if (s >= 0 && e > s) {
      try {
        const norm = normalizeImport(JSON.parse(cleaned.slice(s, e + 1)));
        if (Object.keys(norm).length) return norm;
      } catch (_e) { /* 可能被截斷 → 往下嘗試修補 */ }
    }
    // 1b) AI 輸出被截斷（少收尾 } 或字串沒結束）→ 從第一個 { 到結尾，補齊後再解析
    if (s >= 0) {
      const j = tryParseLooseJson(cleaned.slice(s));
      if (j && typeof j === "object" && !Array.isArray(j)) {
        const norm = normalizeImport(j);
        if (Object.keys(norm).length) return norm;
      }
    }
    // 2) 最後退路：逐行「標籤：值」(自動去掉殘留的引號與結尾逗號，半個 JSON 也救得回來)
    const obj = {};
    text.split(/\r?\n/).forEach((line) => {
      const m = line.match(/^\s*[-*•]?\s*"?\s*([^":：]+?)\s*"?\s*[:：]\s*(.+?)\s*$/);
      if (m) {
        const key = m[1].trim();
        const val = m[2].trim().replace(/,\s*$/, "").replace(/^"([\s\S]*)"$/, "$1");
        obj[key] = val;
      }
    });
  ...
  // 只透過 onAuthStateChange 處理登入狀態：v2 註冊後會立即送出 INITIAL_SESSION，
  // 已涵蓋初次載入，故不再額外呼叫 getSession()，避免初次載入重複觸發 onSession（見上方說明）。
  //
  // ⚠️ 關鍵：onAuthStateChange 的回呼是在 Supabase「認證鎖」鎖著時被呼叫的。
  // 若在回呼內直接再呼叫 sb.auth.getUser()（onSession 裡有用到）會搶同一把鎖，
  // 而鎖正被 signInWithPassword 握著等回呼跑完 → 互等死結（症狀：按登入後永遠卡在「登入中…」、不報錯）。
  // 解法（Supabase 官方建議）：用 setTimeout(…,0) 把後續處理排到鎖釋放之後再執行。
  sb.auth.onAuthStateChange((_e, session) => { setTimeout(() => handleAuth(session), 0); });
})();
